<div class="profile-list-item shadow">
    ...
    @endphp
    <img src="{{ isset($user->avatar) ? upload_asset($user->avatar) : asset('assets/img/avatar/default.png') }}" alt="" class="avatar-img">
    <span class="registrant-name-kana">{{ $user->name_kana }}</span>
    @if($user->profile)
        <div class="job-type">
            <img src="{{ asset('assets/img/icon-light.png') }}">
            {{ implode(', ', array_column($user->profile->jobs->toArray(), 'name')) }}
        </div>
        <div class="week">
            <img src="{{ asset('assets/img/icon-calendar.png') }}">
            週{{ $user->profile->week }}日
        </div>
        <div class="contract-type">
            <img src="{{ asset('assets/img/icon-handshake.png') }}">
