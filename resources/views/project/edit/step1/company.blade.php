<form method="POST" role="form" action="{{ isset($project) ? route('company.project.update') : route('company.project.post') }}" enctype="multipart/form-data" id="form">
    ...
    <div class="form-group">
        @if(isset($project))
            <img src="{{ $project->image ? upload_asset($project->image) : asset('assets/img/icon-image.png') }}" alt="" class="image-upload object-cover-center">
        @else
            <img src="{{ asset('assets/img/icon-image.png') }}" alt="" class="image-upload object-cover-center">
        @endif
    </div>
