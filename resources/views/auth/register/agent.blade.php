        ...
        <div class="col-md-6" style="background-color: #f7f6f3">
            <div class="container">
                <div class="row">
                    <div class="col-md-3">
                        <a href="{{ route('home') }}">
                            <img src="{{ asset('assets/img/logo/logo.png') }}" alt="logo" class="pt-4">
                        </a>
                    </div>
                        ...
                        <div class="content-member-input">
                            ...
                            <div class="step-content{{ (session('step') && session('step') === 1) || !session('step') ? ' active': '' }}" data-step="1">
                                <form method="POST" role="form" action="{{ route('register') }}" enctype="multipart/form-data" id="form">
                                    ...
                                    <div class="d-flex justify-content-center avatar-picker">
                                        <img src="{{ old('avatarPath') ? upload_asset(old('avatarPath')) : asset('assets/img/avatar/default.png') }}" alt="" class="avatar-img">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="password" class="form-control{{ $errors->has('password_confirmation') ? ' is-invalid' : '' }}" id="password-confirm"  name="password_confirmation" placeholder="上記同様のパスワードを入力" required data-parsley-equalto="#password" data-parsley-required-message="半角英数字記号6文字以上20文字以内で入力してください" data-parsley-pattern-message="半角英数字記号6文字以上20文字以内で入力してください" data-parsley-pattern="/^(?=.*[A-Za-z])(?=.*\d)(?=.*\W)[A-Za-z\d\W]{6,20}$/">
                            ...
                            <div class="step-content{{ session('step') && session('step') === 3 ? ' active': '' }}" data-step="3">
                                ...
                                <div class="member-input-btn text-center">
                                    ...
                                    <p><a href="{{ route('agent.dashboard') }}" class="btn btn-black justify-content-center align-items-center">ダッシュボードへ</a></p>
                                </div>
                            </div>
                        </div>
                    </div>
...
@section('style')
    <link rel="stylesheet" href="{{ asset('assets/lib/custom-focus-input/style.css') }}">
@endsection

@section('script')
    <script src="{{ asset('assets/js/page-register.js') }}"></script>
    <script src="{{ asset('assets/lib/custom-focus-input/script.js') }}"></script>
@endsection
